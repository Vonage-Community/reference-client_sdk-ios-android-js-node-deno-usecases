...
import {
    SupabaseUserDeleteWebhook,
    SupabaseUserInsertWebhook,
    SupabaseUserWebhookSchema,
} from '../supabaseDBHookEvents.ts';
...
const logger = getWebhookLogger('new-user');
...
serve(async (req) => {
    ...
    async function deleteVonageUser(webhook: SupabaseUserDeleteWebhook) {
        const vonage_user_id = webhook.old_record.raw_user_meta_data
            ?.vonage_user_id;
        if (!vonage_user_id) {
        ...
        const response = await fetch(
            `https://api.nexmo.com/v0.3/users/${vonage_user_id}`,
            {
                method: 'DELETE',
                headers: {
                    Authorization: `Bearer ${token}`,
