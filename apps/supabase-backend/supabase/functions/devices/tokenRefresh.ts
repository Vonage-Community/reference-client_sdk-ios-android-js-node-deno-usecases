import { z } from 'zod';
import { mintDeviceRefreshToken, mintVonageToken, verifyDeviceRefreshToken } from '../token.js';
...
export const refreshToken = async (client: ReturnType<typeof AdminClient>, data: z.infer<typeof RefreshSchema>): Promise<Response> => {
    try {
        ...
        const { data: device_id, error: deviceIdErr } = await client.from('devices').select('id').eq('id', tokenData.deviceId).eq('user_id', tokenData.userId).single();
        ...
        const { data: username, error: usernameErr } = await client.from('user_profile').select('email').eq('user_id', tokenData.userId).single();
        ...
        const refreshToken = await mintDeviceRefreshToken(Deno.env.get('DEVICE_REFRESH_TOKEN_SECRET') ?? '', tokenData);
        ...
        if (err === InvalidTokenError) return new Response(
