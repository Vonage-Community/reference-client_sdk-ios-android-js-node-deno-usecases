...
export const makeDeviceCode = async (client: ReturnType<typeof UserClient>, data: Device) => {
    const { data: deviceCode, error: deviceCodeErr } = await client.rpc('make_device_code', { device: data.id, salt: DEVICE_CODE_SALT });
